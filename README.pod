...
for example, on debian, 以debian环境为例
    ...
    apt install parallel calibre cpanminus chromium-browser
    cpanm Novel::Robot

=head2 EXAMPLE

=for test t/01.get_novel_info.t t/02.parser_txt.t t/03.packer.t
...
=for test t/01.get_novel_info.t
...
绿晋江 VIP 可通过 C<--cookie> 指定 Netscape cookies.txt、Firefox cookies.sqlite，或直接传入 Cookie 字符串。
...
=for test t/02.parser_txt.t t/03.packer.t
...
use L<calibre|https://calibre-ebook.com/> to convert a novel file into epub/mobi/..., default filename format is [writer]-[bookname].[type]

使用calibre将下载的 html 格式小说转换成其他格式的电子书，例如 epub、mobi 等。如果未指定 writer 及 book 选项，则需要将 html 源文件名称设置为 [作者-书名]

=for test t/03.packer.t
...
本地 SMTP 服务使用 C<-T> 和 C<-F>；远程 SMTP 服务另外使用 C<-M>、C<-p>、C<-U> 和 C<-P>。邮箱、账号及密码必须替换为真实配置，因此不提供不可直接运行的占位示例。

=head2 bulk download

下载专栏内的所有小说：

=for test t/04.board_jjwxc.t
