...
=head2 ARG
    ...
    --use_firefox      : use firefox dump_dom, only support GET method
...
使用calibre将下载的 html格式 的小说转换成 其他格式的电子书，例如epub、epub等等。如果未指定writer及book选项，则需要将html源文件名称设置为 [作者-书名]
    ...
    get_novel.pl -f [input_file] -o [output_file] -t [type] -w [writer] -b [book]
    get_novel.pl -f [源文件] -o [目标文件] -t [目标文件类型(小写)] -w [作者] -b [书名]

    get_novel.pl -f 天平-风起阿房.html -t epub
    get_novel.pl -f mxj.html -w 施定柔 -b 迷侠记 -t epub
