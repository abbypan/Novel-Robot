...
=head2 download and split novel into pieces, each piece contains $n chapters 分段下载小说
...
=head2 convert txt, parse chapter name, add toc 转换txt，加目录
...
=head2 bulk download novels/threads 批量下载小说
...
=head2 only print info, but not download 输出小说信息（不下载）
...
=head2 push novel/thread to wordpress website 将小说发布到wordpress
