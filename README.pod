...
jjwxc vip: 可以使用firefox浏览器，先登录m.jjwxc.net，然后用 [cookies.txt](https://addons.mozilla.org/zh-CN/firefox/addon/cookies-txt/) 扩展导出cookies.txt，则可以下载当前登录账号所购买的小说；也可直接指定firefox配置目录下的cookies.sqlite文件；或者直接使用m.jjwxc.net的cookie字符串
