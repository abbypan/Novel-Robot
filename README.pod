...
L<Novel::Robot> download novel 小说下载器
...
=head2 批量处理小说(支持to txt/html/...)
    ...
    get_novel.pl -w [writer_url] -m [select_menu_or_not] -t [packer_type]
    get_novel.pl -s [site] -q [query_type] -k [query_keyword] -m [select_menu_or_not] -t [packer_type]

    get_novel.pl -w "http://www.jjwxc.net/oneauthor.php?authorid=3243" -m 1 -t html
...
=head2 下载作者专栏
    ...
    my ($writer_name, $books_ref) = $xs->{parser}->get_writer_ref($writer_url, %opt);
    $xs->get_book($_, %opt) for @$books_ref;
...
=head2  查询并下载
    ...
    $xs->get_book($_, %opt) for @$items_ref;
...
=head2  下载版块
    ...
    $xs->get_book($_, %opt) for @$tiezis_ref;

=head2 select_book 在Term下选择小说

    my $select_books_ref = $xs->select_book($banner_info, $books_ref);
